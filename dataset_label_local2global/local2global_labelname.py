...
@Brief   :   local label mapping to global label, 输出label 名称
'''
...
def buildGlobalLabelSys(fname="curr_global_labels.txt"):
    ...
    for cls_idx, cls_info in enumerate(file.readlines()):
        ...
        label_sys[cls_name][modality.upper()] = cls_info
    file.close()
    return label_sys
...
if __name__ == "__main__":
    ...
    for dataset in dataset_list:
        ...
        for idx, k in json_label.items():
                ...
                if(modality.upper() not in global_label_sys[query_k]):
                    ...
                    if(g_key):
                        ...
                        if(modality.upper() not in global_label_sys[query_k]):
                            ...
                            json_per_dataset[ori_key] = str(ori_key)+" "+str(modality)+" not found"
                            continue
                        # json_per_dataset[idx] = global_label_sys[query_k][modality.upper()]
                        json_per_dataset[ori_key] = global_label_sys[query_k][modality.upper()].strip()
                        is_find = True
                    else:
                        json_per_dataset[ori_key] = str(ori_key)+" "+str(modality.upper())+" not found"
            ...
            if(not is_find): 
                ...
                json_per_dataset[ori_key] = str(ori_key)+" not found"
            # else:
    ...
    save_json(json_all, osp.join("/mnt/cache/wanghaoyu/SP_script/dataset_label_local2global/label_name_mapping.json"))
