...
class nnUNetTrainer(NetworkTrainer):
    ...
    # 给nnUNetTraniner 添加一个不会被其他过程使用的新函数，专门用在自定义的Multi-scale Test中
    def preprocess_patient_with_ratio(self, input_files, ratio):
        """
        Used to predict new unseen data. Not used for the preprocessing of the training/test data
        :param input_files:
        :param ratio (3d list) 对spacing的缩放比例
        :return:
        """
        from nnunet.training.model_restore import recursive_find_python_class
        preprocessor_name = self.plans.get('preprocessor_name')
        ...
        spacing_curr = self.plans['plans_per_stage'][self.stage]['current_spacing']
        spacing = spacing_curr*ratio
        print("using spacing with ratio", ratio, spacing)
        d, s, properties = preprocessor.preprocess_test_case(input_files, spacing)
        return d, s, properties
