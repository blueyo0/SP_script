...
def preprocess_multithreaded(trainer, list_of_lists, output_files, num_processes=2, segs_from_prev_stage=None, ratio=None):
    ...
    if(ratio):
        for i in range(num_processes):
            pr = Process(target=preprocess_save_to_queue, args=(trainer.preprocess_patient_with_ratio, q,
                                                                list_of_lists[i::num_processes],
                                                                output_files[i::num_processes],
                                                                segs_from_prev_stage[i::num_processes],
                                                                classes, trainer.plans['transpose_forward'], ratio))
            pr.start()
            processes.append(pr)
    else:
        for i in range(num_processes):
...
def predict_from_folder(model: str, input_folder: str, output_folder: str, folds: Union[Tuple[int], List[int]],
                        ...
                        segmentation_export_kwargs: dict = None, disable_postprocessing: bool = False, ratio: list =None):
    """
    ...
    if mode == "normal":
        ...
        return predict_cases(model, list_of_lists[part_id::num_parts], output_files[part_id::num_parts], folds,
                             ...
                             disable_postprocessing=disable_postprocessing, ratio=ratio)
    elif mode == "fast":
        if overwrite_all_in_gpu is None:
            all_in_gpu = False
        else:
