...
if __name__ == '__main__':
    ...
    # output_files = [join(output_folder, model, "ep"+epoch, tf) for tf in test_files]
    predict_cases(join(parameter_folder, model), [[i] for i in input_files], output_files, folds, save_npz=False,
                    ...
                    checkpoint_name="fp32_model_ep_{:03d}".format(int(epoch)), mode="local")
    ...
    if(osp.exists(general_split_root)):
        ...
        print(general_split_root, "not found")
        print("use all data w/o splits.pkl instead")
